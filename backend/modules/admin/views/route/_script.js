...
$('#btn-new').click(function () {
    ...
    if (route != '') {
        $this.children('i.spinner-border').show();
        $.post($this.attr('href'), {route: route}, function (r) {
            $('#inp-route').val('').focus();
            updateRoutes(r);
        }).always(function () {
            $this.children('i.spinner-border').hide();
        });
    }
    return false;
});
